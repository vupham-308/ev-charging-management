...
const ChargingRates = () => {
  ...
  const fetchRates = async () => {
    try {
      ...
      const formatted = res.data.map((item) => ({
        ...
        type: item.portType,
        cost: item.cost,
        power: item.power, // số nguyên để sửa
        powerLabel: item.power ? item.power + " kW" : "N/A", // để hiển thị
      }));
      setRates(formatted);
    } catch {
  ...
  const saveRate = async () => {
    let port = rateForm.portType.trim().toUpperCase();

    const payload = {
      portType: port,
      power: Number(rateForm.capacity),
    ...
    try {
      if (editRate) {
        ...
        const exists = rates.find((r) => r.type === port);
        if (exists) {
          alert("Loại cổng này đã tồn tại!");
          return;
        }
  ...
  const deleteRate = async (id) => {
    if (!window.confirm("Bạn có chắc muốn xóa giá trạm sạc này?")) return;

    try {
      await api.delete(`charger-cost/${id}`);
      alert("Xóa thành công!");
      fetchRates();
    } catch (err) {
      console.log(err);
      alert("Không thể xóa giá trạm sạc!");
    }
  };
      ...
      >
        <div>
          ...
          <p className="text-gray-600 mb-0">
            Điều chỉnh giá sạc theo loại cổng.
          </p>
        </div>
      ...
      {rates.map((rate) => (
        <div className="rate-card" key={rate.id}>
          <div className="rate-info">
            ...
            <div className="rate-text">
              <div className="rate-header">
                <span>{rate.powerLabel}</span>
                <span className="rate-type">{rate.type}</span>
              </div>
              <p className="rate-price">{rate.cost} đ/kWh</p>
            </div>
          </div>
          ...
          <div className="rate-actions">
            <button
              className="btn-edit"
              onClick={() => openEditRatePopup(rate)}
            >
              <FaEdit /> Sửa
            </button>
        ...
        <div className="rate-card" key={brand.id}>
          <div className="rate-info">
            ...
            <div>
              ...
              <p className="battery">
                Dung lượng pin: {brand.batteryCapacity || brand.battery} kWh
              </p>
            </div>
          </div>
          ...
          <div className="rate-actions">
            <button
              className="btn-edit"
              onClick={() => openEditBrandPopup(brand)}
            >
              <FaEdit /> Sửa
            </button>
            <button
          ...
          <div className="brand-popup">
            ...
            <div className="popup-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setRateForm({ ...rateForm, portType: e.target.value })
                }
                placeholder="Ví dụ: AC, CCS, CHAdeMO..."
              />
            </div>
            ...
            <div className="popup-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setRateForm({ ...rateForm, capacity: e.target.value })
                }
            ...
            <div className="popup-field">
              ...
              <input
                ...
                value={rateForm.cost}
                onChange={(e) =>
                  setRateForm({ ...rateForm, cost: e.target.value })
                }
      ...
      {showBrandPopup && (
        <div className="popup-overlay">
          <div className="brand-popup">
            ...
            <h2>
              {editBrand ? "Chỉnh sửa thương hiệu xe" : "Thêm thương hiệu xe"}
            </h2>
            ...
            <div className="popup-field">
              ...
              <select
                ...
                onChange={(e) =>
                  setBrandForm({ ...brandForm, portType: e.target.value })
                }
              >
            ...
            <div className="popup-field">
              ...
              <input
                ...
                onChange={(e) =>
                  setBrandForm({ ...brandForm, battery: e.target.value })
                }
              />
            </div>
