...
const StationDetail = () => {
  ...
  const fetchPortTypes = async () => {
    try {
      ...
      console.error("Lỗi tải portType:", e);
    }
  };
  ...
  const fetchChargers = async () => {
    const res = await fetch(
      `http://222.255.214.35:8080/api/chargerPoint/getAll/${id}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
    ...
    const simulatedData = data.map((charger) => {
      if (charger.name === "Trụ #1") {
        return {
          ...
          sessionStartTime: "2025-11-02T14:30:00",
        };
      }
      if (charger.name === "Trụ #3") {
        return {
          ...
          sessionStartTime: "2025-11-02T15:00:00",
        };
      }
      if (charger.name === "Trụ #7") {
        return {
          ...
          sessionStartTime: "2025-11-02T13:45:00",
        };
      }
      if (charger.name === "Trụ #8") {
        return {
          ...
          sessionStartTime: "2025-11-02T14:15:00",
        };
      }
    ...
    const available = simulatedData.filter(
      (c) => c.status?.toUpperCase() === "AVAILABLE"
    ).length;
    const inUse = simulatedData.filter((c) =>
      ["IN_USE", "OCCUPIED"].includes(c.status?.toUpperCase())
    ).length;
    const maintenance = simulatedData.filter(
      (c) => c.status?.toUpperCase() === "OUT_OF_SERVICE"
    ).length;
    const reserved = simulatedData.filter(
      (c) => c.status?.toUpperCase() === "RESERVED"
    ).length;
  ...
  const fetchStationStats = async () => {
    try {
      const res = await fetch(
        `http://222.255.214.35:8080/api/station/admin/dashboard-status/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      message.success(
        editingCharger ? "Cập nhật thành công!" : "Thêm mới thành công!"
      );
      setIsModalVisible(false);
      fetchChargers();
    } catch (e) {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: "auto" }}>
        ...
        <Form form={form} layout="vertical">
          ...
          >
            <Select placeholder="Chọn loại cổng sạc">
              {portTypes.map((pt) => (
                <Select.Option key={pt.id} value={pt.portType}>
                  {pt.portType} - {pt.power} kW
                </Select.Option>
              ))}
            </Select>
          </Form.Item>
      ...
      <Card style={{ borderRadius: 10, marginBottom: 24 }}>
        ...
        <p>📍 {station.address}</p>
        <p>
          <PhoneOutlined /> {station.phone} &nbsp;&nbsp; <MailOutlined />{" "}
          {station.email}
        </p>
          ...
          {chargers.map((c) => (
            <Card key={c.id}>
              ...
              {["IN_USE", "OCCUPIED"].includes(c.status?.toUpperCase()) && (
                ...
                >
                  <UserOutlined /> {c.currentUser}
                  <br />
                  <ClockCircleOutlined /> Bắt đầu:{" "}
                  {formatTime(c.sessionStartTime)}
                </div>
              )}
      ...
      <Modal
        ...
        onOk={async () => {
          try {
            const res = await fetch(
              `http://222.255.214.35:8080/api/chargerPoint/admin/delete?chargerPointId=${chargerToDelete.id}`,
              {
                method: "DELETE",
                headers: {
                  Authorization: `Bearer ${token}`,
            ...
            if (!res.ok) throw new Error("Xóa trụ thất bại!");
      ...
      >
        <p>
          Bạn có chắc chắn muốn xóa trụ sạc <b>{chargerToDelete?.name}</b>? Hành
